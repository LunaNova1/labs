...
def update(drone):
    ...
    if largest is None:
        ...
        center = uav_utils.get_contour_center(largest)
        row_offset = center[0] - ROW_CENTER
        col_offset = center[1] - COL_CENTER
        pitch = uav_utils.clamp(row_offset, -MAX_TILT, MAX_TILT)
        roll = uav_utils.clamp(col_offset, -MAX_TILT, MAX_TILT)
        drone.flight.send_pcmd(pitch, roll, 0, 0)

        if abs(row_offset) < CENTER_TOL and abs(col_offset) < CENTER_TOL:
