...
MIN_PTS = 5         # re-find features when fewer than this survive
...
def update(drone):
    ...
    if _frame % SKIP == 0:
        ...
        if _prev_gray is None or _prev_pts is None or len(_prev_pts) < MIN_PTS:
            print(f"[re-detect] prev_pts count = {0 if _prev_pts is None else len(_prev_pts)}")
