...
def update(drone):
    ...
    if _frame % SKIP == 0:
        speed = 0
        image = drone.camera.get_downward_image()   # or whichever camera this module uses
        ...
        if _prev_pts is None or len(_prev_pts) < MIN_PTS:
            ...
            _last_mag = displacements.mean()
            height = drone.physics.get_altitude()
            meters_per_pixel = (2 * height * HFOV_TAN) / IMAGE_WIDTH
            speed = (_last_mag * meters_per_pixel) / _interval
            _interval = 0.0
            _prev_gray = gray
    
        if _timer >= RUN_TIME:
            print(f"actual - {drone.physics.get_linear_velocity()}, estimated - {speed}")
            _done = True
