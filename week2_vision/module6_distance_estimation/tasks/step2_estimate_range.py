...
APPROACH_PITCH = 0.25     # forward speed while approaching
...
def update(drone):
    ...
    if gate is None:
        drone.flight.send_pcmd(0, 0, SEARCH_YAW, 0)
         ...
         col_offset = (x + w / 2) - COL_CENTER
         yaw = uav_utils.clamp(col_offset * YAW_GAIN, -MAX_YAW, MAX_YAW)
         print(f"w={w}, distance={distance:.2f}, col_offset={col_offset}, yaw={yaw}")
